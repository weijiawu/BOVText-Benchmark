...
_EVENT_FIELDS = ['Type', 'OId', 'HId', 'D','Rec']
...
class MOTAccumulator(object):
    ...
    def _append_to_events(self, typestr, oid, hid, distance, rec):
        self._events['Rec'].append(rec)
    ...
    def update(self, oids, hids, dists, frameid=None, vf=''):
        ...
        self._append_to_events('RAW', np.nan, np.nan, np.nan, 'RAW')
        ...
        for i, j, dist_ij in zip(valid_i, valid_j, valid_dists):
            self._append_to_indices(frameid, next(eid))
            self._append_to_events('RAW', oids[i], hids[j], dist_ij, 'RAW')
        ...
        for oid in oids[unused_i]:
            self._append_to_indices(frameid, next(eid))
            self._append_to_events('RAW', oid, np.nan, np.nan, 'RAW')
        for hid in hids[unused_j]:
            self._append_to_indices(frameid, next(eid))
            self._append_to_events('RAW', np.nan, hid, np.nan, 'RAW')
        ...
        if oids.size * hids.size > 0:
            ...
            for i in range(oids.shape[0]):
                ...
                hprev = self.m[oids[i]] # 得到对应det的id
                ...
                if np.isfinite(dists[i, j]):
                    ...
                    self._append_to_indices(frameid, next(eid))
                    self._append_to_events('MATCH', oids[i], hids[j], dists[i, j], 'tp')
...
#             assert False
            ...
            for i, j in zip(rids, cids):
                ...
                if cat1 == 'SWITCH':
                    if h not in self.hypHistory:
                        subcat = 'ASCEND'
                        self._append_to_indices(frameid, next(eid))
                        self._append_to_events(subcat, oids[i], hids[j], dists[i, j], 'tp')
                ...
                if cat2 == 'TRANSFER':
                    if o not in self.last_match:
                        ...
                        self._append_to_events(subcat, oids[i], hids[j], dists[i, j], 'tp')
                    self._append_to_indices(frameid, next(eid))
                    self._append_to_events(cat2, oids[i], hids[j], dists[i, j], 'tp')
                ...
                self.last_match[o] = frameid
                self._append_to_indices(frameid, next(eid))
                self._append_to_events(cat1, oids[i], hids[j], dists[i, j], 'tp')
                oids_masked[i] = True
                hids_masked[j] = True
                self.m[o] = h
                self.res_m[h] = o
        ...
        for o in oids[~oids_masked]:
            self._append_to_indices(frameid, next(eid))
            self._append_to_events('MISS', o, np.nan, np.nan, 'fn')
            if vf != '':
                vf.write('FN %d %d\n' % (frameid, o))
        ...
        for h in hids[~hids_masked]:
            self._append_to_indices(frameid, next(eid))
            self._append_to_events('FP', np.nan, h, np.nan, 'fp')
            if vf != '':
                vf.write('FP %d %d\n' % (frameid, h))
    ...
    def new_event_dataframe():
        ...
        df = pd.DataFrame(
            OrderedDict([
                ...
                ('Rec', pd.Series(cats)),          # Type of event. One of FP (false positive), MISS, SWITCH, MATCH
    ...
    def new_event_dataframe_with_data(indices, events):
        ...
        series = [
            ...
            pd.Series(raw_type_rec, name='Rec'),
