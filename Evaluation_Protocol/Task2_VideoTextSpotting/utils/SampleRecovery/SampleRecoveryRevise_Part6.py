...
def load_4points4items(file):
    '''  x1,y1,x2,y2,x3,y3,x4,y4, extraContent, option, object_id, frame_id '''
    data_list = []
    with open(file, 'r', encoding='utf-8') as fr:
        for line in fr.readlines():
            line = line.strip('\n')
            data = line.split(':')
            if len(data) != 12 or data[8] == "#1": # or data[8] == '#nuII':
                continue
            data_list.append(data)
    return data_list
...
def IsInCluster(frame_id, obj_id, TL_Cluster_dict, data, TIoU=0.3, FrameGap=3, TSimEdit=0.5):
    '''  判断新文字目标是否可以合并到已有类簇中
    ...
    for cluster_key, cluster_dict in TL_Cluster_dict.items():
        ...
        if frame_id - int(end_frame_id) > FrameGap or frame_id == int(end_frame_id):
            continue
...
#             return True
#     elif max_sim>TSimEdit and max_iou_sim>0.001:
...
#         return False
    ...
    if current_Cls == '背景文字':
        ...
        if max_iou > 0.5 and max_sim_iou>0.5:
            ...
            TL_Cluster_dict[max_cluster_key]["element_list"].append(new_data)
            return True
        elif max_sim>0.98 and max_iou_sim>0.001:
            TL_Cluster_dict[max_cluster_strkey]["cluster_center"] = new_data
            TL_Cluster_dict[max_cluster_strkey]["end_frame_id"] = frame_id
            TL_Cluster_dict[max_cluster_strkey]["element_list"].append(new_data)
            return True
...
def ComplementSampleFrames(TL_Cluster_dict):
    '''
        对采样空白区域进行补充：
        1、补充采样点之间的文字；
    '''
...
def Cluster2Frames(TL_Cluster_dict, Frames_dir):
    ...
    Cls_GtTxt_dir = Cls_dir.replace('_Frames','_GtTxtsR4Frames')
    ...
    print('Cluster4Frames()==>',len(TL_Cluster_dict), NumFrames)
    FramesGt_dict = {}
    ...
    for cluster_id, cluster_dict in TL_Cluster_dict.items():
        ...
        for data in cluster_dict['element_list']:
            #print(data, 'Cluster2Frames')
            #if int(data[-1])>NumFrames:
            #    print('Part of Cluster2Frames()==>',data)
            #    continue
            data[-2] = cluster_id     #同一个文字赋值同一个类簇id
            FramesGt_dict[int(data[-1])].append(data)
...
def ReviseSingleFrameAnnotation(data_list):
    '''
    data = x1,y1,x2,y2,x3,y3,x4,y4, extraContent, option, obj_id, frame_id
    ...
    # Fixme: 1、采用iou去掉重叠文字框
    for data in data_list:
        x1, y1, x2, y2, x3, y3, x4, y4, extraContent, option, obj_id, frame_id = data
        box = [x1, y1, x2, y2, x3, y3, x4, y4]
        box = [int(e) for e in box]
        x1, y1, x2, y2, x3, y3, x4, y4 = box
        area = Polygon(np.array([[x1, y1], [x2, y2],[x3, y3], [x4, y4]])).area
        box.append(area)
        re_data_list.append(box)

    re_data_list = LA_NMS.nms_locality(re_data_list, thres=0.8)
    #for data in re_data_list:
    #    print('ReviseSingleFrameAnnotation',data)
    re2_data_list = []
    for idx, data in enumerate(re_data_list):
        x1, y1, x2, y2, x3, y3, x4, y4, area = [int(e) for e in data]
        box = [x1, y1, x2, y2, x3, y3, x4, y4]
        iou_max = 0.0
        max_idx = 0
        for idx2, data2 in enumerate(data_list):
            x1_p, y1_p, x2_p, y2_p, x3_p, y3_p, x4_p, y4_p, extraContent, option, obj_id, frame_id = data2
            box2 = [x1_p, y1_p, x2_p, y2_p, x3_p, y3_p, x4_p, y4_p]
            box2 = [int(e) for e in box2]
            c_iou = LA_NMS.intersection(box, box2)
            if c_iou > iou_max:
                iou_max = c_iou
                max_idx = idx2
        x1_p, y1_p, x2_p, y2_p, x3_p, y3_p, x4_p, y4_p, extraContent, option, obj_id, frame_id = data_list[max_idx]
        re2_data_list.append([x1, y1, x2, y2, x3, y3, x4, y4, extraContent, option, obj_id, frame_id])

    #print('ReviseSingleFrameAnnotation()', len(re2_data_list))
    # Fixme: 2、采用 包含判断 去掉不统一定义
    rre_data_list = []#copy.deepcopy( re2_data_list )
    del_list = []
    num = len(re_data_list)
#     for idx, data in enumerate(re2_data_list):
#         x1, y1, x2, y2, x3, y3, x4, y4, extraContent, option, obj_id, frame_id = data
#         box = [x1, y1, x2, y2, x3, y3, x4, y4]
#         for idx2 in range(idx+1, num):
#             x1_p, y1_p, x2_p, y2_p, x3_p, y3_p, x4_p, y4_p, extraContent_p, option_p, obj_id_p, frame_id_p = re2_data_list[idx2]
#             box2 = [x1_p, y1_p, x2_p, y2_p, x3_p, y3_p, x4_p, y4_p]
# #             if LA_NMS.intersection(box, box2) > 0.1:
# #                 if extraContent_p in extraContent:
# #                     print(extraContent_p)
# #                     print(extraContent)
...
#             continue
#         else:
#             rre_data_list.append(data)
    #print('ReviseSingleFrameAnnotation()', len(rre_data_list), len(del_list)) 

    return re2_data_list

def ReviseVideoAnnotations(FramesAnno_dir, Frames_dir):
    '''
    后处理单个视频：
    1、采用iou去掉重叠文字框；
    2、采用包含判断去掉不统一定义；
    ...
    4、统一文字id；
    '''
    # FramesAnno_dir = "E:\\KSText\\KSText_VideoFrames_Results\\Cls5_PP_GtTxtsFrames\\39734424643"
    # Frames_dir = "E:\\KSText\\KSText_VideoFrames_Results\\Cls5_PP_Frames\\39734424643"
    print('ReviseVideoAnnotations()',FramesAnno_dir)
    print('ReviseVideoAnnotations()',Frames_dir)
    Frames_GtTxt_list = get_file_path_list(FramesAnno_dir, ['.txt'])
    Frames_GtTxt_list.sort()
    TL_dict = {}
    TL_Cluster_dict = {}
    TL_Cluster_dict["cluster_num"] = 0
    for GtTxt_path in Frames_GtTxt_list:
        frame_name = os.path.split(GtTxt_path)[1]
        frame_id = int(os.path.splitext(frame_name)[0].split('_')[1])
        data_list = load_4points4items(GtTxt_path)
        #print('ReviseVideoAnnotations()==>',frame_id, len(data_list))
        data_list = ReviseSingleFrameAnnotation(data_list)
        #print('ReviseVideoAnnotations()==>',frame_id, len(data_list))
        #for data in data_list:
        #    print(data)
        ...
        for obj_id, data in enumerate(data_list):
            data = data[:10]
            #print('ReviseVideoAnnotations()==>',frame_id, obj_id, data)
            #TL_dict[frame_id][obj_id] = data
            isLink = IsInCluster(frame_id, obj_id, TL_Cluster_dict, data)
            if isLink == True:
    ...
    #采样标注恢复为逐帧标注
    ComplementSampleFrames(TL_Cluster_dict)
...
def BatchReviseVideoAnnotations():
    '''

    '''
    AllFrames_dir = "/share/caiyuanqiang/VideoSet/"
...
#         "Cls21_MY","Cls22_QC","Cls23_HW","Cls24_YY","Cls25_DJ"
    ...
    for Cls_name in ClsName_list:
        ...
        ClsFramesDir_list = []
        ClsSampleGtTxtFrames_dir = os.path.join(AllFrames_dir, Cls_name)+"_GtTxtsR3Frames"
...
#             print(os.path.join(ClsFrames_dir, video_pre))
#             print(os.path.join(ClsSampleGtTxtFrames_dir, video_pre))
#             assert False
        ...
        for idx, ClsFrames_path in enumerate(ClsFramesDir_list):
            ...
            if not os.path.exists(ClsSampleGtTxtFrames_path):
                ...
                continue
            # ClsFrames_path 存储类别下的一个视频的所有视频帧
            # ClsSampleGtTxtFrames_path 存储类别下的一个视频的所有视频帧的标注信息
            ReviseVideoAnnotations(FramesAnno_dir=ClsSampleGtTxtFrames_path, Frames_dir=ClsFrames_path)
...
if __name__ == "__main__":
    ...
    # parse()
    # BatchParse()
    # ReviseVideoAnnotations()
    BatchReviseVideoAnnotations()
