"""Compute metrics for text trackers using Text MOTChallenge ground-truth data."""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="evaluate task2 end to end video text spotting", formatter_class=argparse.RawTextHelpFormatter)

    parser.add_argument('--groundtruths', type=str,default=''
...
def iou_matrix_polygen(objs, hyps, gt_contents,trk_contents, max_iou=1.):
...
#     print(objs)
    ...
    for row in range(m):
        for col in range(n):
            gt_content = gt_contents[row]
            trk_content = trk_contents[col]
            iou = calculate_iou_polygen(objs[row], hyps[col])
            dist = 1-iou
            if gt_content!=trk_content:
                dist = np.nan
...
class Evaluator(object):
    ...
    def eval_frame(self, frame_id, trk_tlwhs, trk_ids,trk_content, rtn_events=False):
        # results
...
#         print(gts)
        ...
        iou_distance = iou_matrix_polygen(gt_tlwhs, trk_tlwhs, gt_content,trk_content, max_iou=0.5)
...
#             trk_objs = result_frame_dict.get(frame_id, [])
            if str(frame_id) in result_frame_dict.keys():
                ...
                trk_content,trk_tlwhs,trk_ids = [], [], []
                for trk_obj in trk_objs:
                    trk_content.append(trk_obj[8])
                    trk_tlwhs.append(trk_obj[:8])
                    trk_ids.append(trk_obj[-1])
                    
#                 trk_content = trk_objs[:,8]
                trk_tlwhs = np.array(trk_tlwhs,dtype=np.int32)
                trk_ids = np.array(trk_ids,dtype=np.int32)
            else:
                trk_tlwhs = np.array([])
                trk_ids = np.array([])
                trk_content = []
...
#             assert False
            self.eval_frame(str(frame_id), trk_tlwhs, trk_ids, trk_content, rtn_events=False)
