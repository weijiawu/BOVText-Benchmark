...
if __name__ == "__main__":
    ...
    for idx,video_path in enumerate(os.listdir(image_path)):
        if video_path not in seqs:
            continue
            
        annotation_path_ = os.path.join(annotation_path,video_path+".json")
        annotation = get_annotation(annotation_path_)
        
        video_path_ = os.path.join(image_path,video_path)
        video_vis_path = os.path.join(vis_path,video_path)
        ...
        for image in tqdm(os.listdir(video_path_)):
            ...
            if image.split(".jpg")[0] in annotation.keys():
                annotatation_frame = annotation[image.split(".jpg")[0]]
...
#                         frame=cv2AddChineseText(frame,id_content, (int(x1), int(y1) - 20),(0, 255, 0), 35)
            ...
            frame_vis_path = os.path.join(video_vis_path, image)
